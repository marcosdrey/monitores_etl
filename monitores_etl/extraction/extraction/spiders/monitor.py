...
class MonitorSpider(scrapy.Spider):
    ...
    start_urls = ["https://lista.mercadolivre.com.br/monitor-gamer?sb=rb#D[A:monitor%20gamer]"]
    page_count = 1
    max_page = 10
    ...
    def parse(self, response):
        ...
        for product in products:
            previous_price = product.css('.andes-money-amount--previous').css(".andes-money-amount__fraction::text").get()
            current_price = product.css('.poly-price__current').css('.andes-money-amount__fraction::text').get()
        ...
        if self.page_count < self.max_page:
            next_page_url = response.css('.andes-pagination__button--next a::attr(href)').get()
            if next_page_url:
                self.page_count += 1
